...
public class WorkerServer extends Thread
{
	...
	public void run()
	{
		...
		while (true)
		{
			try
			{
				Socket masterSocket = workerServerSocket.accept();
	...
	private static class WorkerCommandHandler implements Runnable
	{
		...
		public void run()
		{
			...
			finally
			{
				...
				{
					// Ignore because we're about to exit
					// anyway.
				}
			}
		}
	...
	/**
	 * Starts a worker server on a distinct port. This information can be
	 * either specified as command line arguments or via system properties
	 * specified in the <code>workers.properties</code> file (if no command
